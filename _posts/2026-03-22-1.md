...
`C++ constexpr`, `zig comptime`, rust의 매크로함수인 `obfstr!`함수를 이용하여 컴파일 타임에 문자열을 난독화한다. 여기에서는 AOT만을 이용하여 기본을 학습한 후 이것을 토대로 `zig, rust, c++`를 라이브러리로 만들어 c#에서 활용해 보려고 한다.
...
위의 소스 중 아래의 소스는 실제 프로젝트에 포함하지 말아야 한다. `Helper.cs`파일도 마찬가지. 난독화한 바이트 배열을 만들고 이것을 AOT 라이브러리 소스에 복사하여 사용한다.
