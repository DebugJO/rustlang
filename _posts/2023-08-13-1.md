...
* 배열로부터 가져올 때: `Span<byte> span = new byte[1024];`
이것은 결국 Managed 배열이고. GC가 관리하며, 대용량일 경우 GC 부하(LOH 등)가 발생한다.

* 스택으로부터 가져올 때: `Span<byte> span = stackalloc byte[1024];`
GC 부하가 없고 빠르지만, 크기 제한이 엄격하며 보통 1MB가 넘어가면 StackOverflowException 발생

* 네이티브 메모리로부터 가져올 때: `NativeMemory.Alloc(...count...)`
가장 빠르고 크기 제한도 없지만, 수동으로 해제(Free) 하지 않으면 메모리 누수(Memory Leak)가 발생
...
|**해제 시점**|GC가 한가할 때 (언젠가)|함수 종료 시 즉시|Dispose 호출 시 즉시|
|**크기 제한**|힙 메모리만큼 (매우 큼)|매우 작음 (1MB 이하 권장)|RAM 용량만큼 (매우 큼)|
|**안전성**|매우 안전함|빠르지만 위험함|수동 해제 필수 (using으로 보완)|
