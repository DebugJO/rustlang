...
그렇지만 러스트(rust)는 예외 기능이 없다. 대신, 복구 가능한 에러를 위한 `Result<T, E>` 값과 복구 불가능한 에러가 발생했을 때 실행을 멈추는 panic! 매크로를 가지고 있다.<sup>1</sup>

또한 러스트는 다른 언어들이 가지는 Null이라는 기능이 없다. 러스트는 Null을 사용하지 않고, 존재하거나 존재하지 않음을 나타내는 개념을 나타내는 `Option<T>`을 이용한다.<sup>2</sup> 
