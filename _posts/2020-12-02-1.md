...
categories: [RustㆍZigㆍGo Tutorial]
---

Rust 프로그래밍에서 concurrency 모델의 핵심 메커니즘을 비교적 간단한 소스 코드를 통하여 단계별로 정리해보았다.<sup>1</sup>

Go(lang)에서 goroutine, channel을 사용하여 함수와 메소드의 동시성을 구현할 수 있게 해주는 것<sup>2</sup>처럼 Rust에서는 'Concurrency, Threads, Channels, Mutex and Arc'로 동시성을 구현할 수 있다.
...
Go(lang)에서 Concurrency 구현 예제는 '[Go, REST API with Mux](https://devsight.kr/2020/10/01/1/)'를 참고하자<sup>5</sup>.
...
> 5. [devsight.kr, "Go, REST API with Mux"](https://devsight.kr/2020/10/01/1/)
