...
프로그램 언어들은 예외 핸들링(exception handling) 또는 반환 값(return value) 이라는 두 가지 에러 핸들링 접근 방식 중 한 가지를 사용한다. Rust는 후자를 사용한다.<sup>1</sup> 이전 글 [Rust 예외 및 에러 처리](https://devsight.kr/2024/05/17/1/)에서 복구 가능한 에러를 위한 `Result<T, E>` 사용법을 살펴봤다.
...
| 용도    | 라이브러리 개발, 세밀한 에러 처리,<br>성능 중시 | 애플리케이션 개발, 빠른 개발,<br>간단한 에러처리 |
